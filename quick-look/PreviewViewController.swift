...
final class PreviewViewController: NSViewController, QLPreviewingController, WKNavigationDelegate {
    ...
    private func makeCopyButton() -> NSButton {
        ...
        button.identifier = NSUserInterfaceItemIdentifier("QuickLookCopyText")
        ...
        button.setAccessibilityHelp(copyTextHelp)
    ...
    @objc private func copyText(_ _: NSButton) {
        guard isPreviewVisible, isPreviewReady, let sourceText else {
            NSSound.beep()
            return
        }
        ...
        let pasteboard = NSPasteboard.general
        pasteboard.clearContents()
        guard pasteboard.setString(sourceText, forType: .string) else {
            NSSound.beep()
            return
        }
