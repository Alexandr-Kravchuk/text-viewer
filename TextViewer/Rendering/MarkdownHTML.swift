...
nonisolated enum MarkdownHTML {
    ...
    static func render(markdown: String,
        ...
        let bodyHTML = """
        <pre class="text-viewer-source" data-source-line="1" data-source-start="1" data-source-end="\(lineCount)">\(htmlEscape(markdown))</pre>
        """
        ...
        article.markdown-body > pre.text-viewer-source {
            margin: 0;
            padding: 0;
            border: 0;
        ...
        let mathBlock = VendorEmission()
        let mermaidBlock = VendorEmission()
        let highlightBlock = VendorEmission()
