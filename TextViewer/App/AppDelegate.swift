    ...
    var errorDescription: String? {
        ...
        case .bundledToolMissing:
            return NSLocalizedString(
                "The bundled Text Viewer CLI could not be found.",
                comment: "CLI installer error"
            )
        }
    }
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    /// A malformed text-viewer:// link tells the reader what shape the app
    ...
    private func presentUnsupportedSchemeURLAlert(_ urls: [URL]) {
        ...
        alert.informativeText = String(
            format: NSLocalizedString(
                "“%@” is not a link Text Viewer understands. Use text-viewer://file/ followed by the absolute path of the file, for example text-viewer://file/Users/me/notes/notes.txt.",
                comment: "URL scheme error"
            ),
    ...
    @objc private func installCommandLineTools(_ sender: Any?) {
        do {
            ...
            NSLog("Failed to run Text Viewer CLI installer in Terminal: \(error.localizedDescription)")
        }
    }
    ...
    private func makeCommandLineToolInstallerScript(commandLineToolURL: URL) -> String {
        ...
        choice=$(choose_install_dir) || {
          echo "Could not find a usable PATH directory for Text Viewer command line tools." >&2
          exit 1
        }

        install_dir=${choice%	*}
        needs_sudo=${choice#*	}
        primary="$install_dir/text-viewer"

        is_text_viewer_launcher() {
          path="$1"
          [ -f "$path" ] && [ ! -L "$path" ] || return 1
          if grep -q '^# Managed by Text Viewer CLI$' "$path"; then
            return 0
          fi
          grep -q 'exec open -a "Text Viewer"' "$path"
        }
        ...
        can_replace_alias() {
          ...
          [ "$alias_target" = "text-viewer" ] ||
            [ "$alias_target" = "$primary" ] ||
            [ "$alias_target" = "$install_dir/text-viewer" ]
        }

        refuse_existing_command() {
          echo "Refusing to replace existing command that was not installed by Text Viewer: $1" >&2
          exit 1
        }
        ...
        if [ "$needs_sudo" = "true" ]; then
          echo "Installing Text Viewer command line tools to $install_dir requires your password."
        ...
        for alias in tv; do
          ...
          if [ "$needs_sudo" = "true" ]; then
            sudo ln -sfn "text-viewer" "$alias_path"
          else
            ln -sfn "text-viewer" "$alias_path"
          fi
        done

        echo
        echo "Text Viewer CLI is ready."
        echo
        ...
        echo
        echo "Examples:"
        echo "  text-viewer system.log  Open a text file"
        echo "  tv .                    Open the current folder"
        echo "  tv logs                 Browse a folder in Text Viewer"
        echo
        echo "Tips:"
        echo "  Use tv for the shortest command."
        ...
        if command -v tv >/dev/null 2>&1; then
          ...
          echo "Open a new terminal window, then try: text-viewer ."
        fi
        """
    }

    private func bundledCommandLineToolURL() throws -> URL {
        guard let url = Bundle.main.url(forResource: "text-viewer",
                                        withExtension: nil,
    ...
    private func writeCommandLineToolInstallerScript(commandLineToolURL: URL) throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("install-text-viewer-cli-\(UUID().uuidString).sh")
    ...
    private func installAppMenuItems() {
        ...
        if let updatesItem = appMenu.items.first(where: { $0.action == NSSelectorFromString("checkForUpdates:") }) {
            appMenu.removeItem(updatesItem)
        }
        ...
        let cliItem = NSMenuItem(title: L("Install CLI..."),
                                 ...
                                 keyEquivalent: "")
        cliItem.target = self
        appMenu.insertItem(cliItem, at: min(1, appMenu.numberOfItems))
    }
