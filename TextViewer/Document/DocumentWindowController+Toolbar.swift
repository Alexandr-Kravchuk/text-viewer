...
extension NSToolbarItem.Identifier {
    ...
    static let copyText = NSToolbarItem.Identifier("CopyText")
...
extension DocumentWindowController {
    ...
    func toolbar(_ toolbar: NSToolbar,
        ...
        case .copyText: return makeCopyItem()
    ...
    private func makeAlwaysOnTopItem(willBeInsertedIntoToolbar: Bool) -> NSToolbarItem {
        ...
        item.toolTip = NSLocalizedString("Keep Text Viewer windows in front of other apps",
    ...
    private func makeCopyItem() -> NSToolbarItem {
        let item = NSToolbarItem(itemIdentifier: .copyText)
        let copy = NSLocalizedString("Copy", comment: "Copy toolbar item label")
        item.label = copy
        item.paletteLabel = copy
        item.toolTip = NSLocalizedString("Copy text to clipboard", comment: "Copy toolbar item tooltip")
        item.image = copyIdleImage()
        item.isBordered = true
        item.target = self
        item.action = #selector(copyTextAction(_:))
        copyItem = item
        return item
    }
