...
enum OpenTargetCatalog {
    ...
    private static let textyUTIs = plainTextUTIs
    ...
    private static let editorBundleIDDenylist: Set<String> = ["com.microsoft.Word"]
    ...
    /// Editors that can open plain text in general, with no document in hand.
    ///
    /// Settings has to offer the choice before any document is open, so it asks
    /// Launch Services by content type rather than by URL. Plain-text editors
    /// register for `public.plain-text` even when no document is selected.
    static func textEditorCandidates() -> [EditorCandidate] {
        var seen: Set<URL> = []
        let urls = NSWorkspace.shared.urlsForApplications(toOpen: .plainText).filter {
            seen.insert(canonicalAppURL($0)).inserted
        }
    ...
    private static func filterEditors(_ appURLs: [URL]) -> [EditorCandidate] {
        ...
        return appURLs.compactMap { appURL in
            ...
            guard isAllowlisted || canEditPlainText(plist: plist) else { return nil }
    ...
    private static func canEditPlainText(plist: [String: Any]?) -> Bool {
        ...
        for docType in docTypes {
            ...
            let hasTextExtension = !TextFileTypes.extensions.isDisjoint(with: extensions)
            ...
            guard hasTextExtension || isPlainTextEditor else { continue }
