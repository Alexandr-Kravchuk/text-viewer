...
extension DocumentMetadata {
    static func make(url: URL?, markdown: String) -> DocumentMetadata {
        ...
        meta.lineCount = markdown.isEmpty ? 0 : lines.count
...
struct InspectorView: View {
        ...
        Form {
            Section {
                ...
                LabeledContent(
                    ...
                    value: NSLocalizedString("Text Document", comment: "Inspector document type")
                )
    ...
    private var propertiesTab: some View {
        Form {
            Section(NSLocalizedString("File", comment: "Inspector properties section")) {
                if let url = metadata.fileURL {
                    LabeledContent(
                        NSLocalizedString("Extension", comment: "Inspector file property"),
                        value: url.pathExtension.isEmpty ? NSLocalizedString("Unknown", comment: "Unknown file extension") : url.pathExtension.uppercased()
                    )
                    LabeledContent(
                        NSLocalizedString("Location", comment: "Inspector file property"),
                        value: Self.displayPath(for: url)
                    )
                }
                if let size = metadata.fileSize {
                    LabeledContent(
                        NSLocalizedString("Size", comment: "Inspector file property"),
                        value: size.formatted(.byteCount(style: .file))
                    )
                }
                if let modified = metadata.modifiedDate {
