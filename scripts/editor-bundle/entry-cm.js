...
window.MDEditor = {
  create(parent, doc, callbacks) {
    ...
    const plainText = callbacks && callbacks.plainText === true
    ...
    const view = new EditorView({
      ...
      state: EditorState.create({
        ...
        extensions: [
          ...
          directionLines,
          ...(plainText ? [] : [
            // Markdown-specific editing stays out of Text Viewer: the text
            // mode preserves exactly what the user types.
            yamlFrontmatter({
              content: markdown({
          ...
          keymap.of([
            ...
            { key: "Tab", run: plainText ? insertTab : indentMarkdownListItems, shift: indentLess },
            ...closeBracketsKeymap,
            ...(plainText ? [] : markdownKeymap),
            ...defaultKeymap,
            ...historyKeymap,
          ]),
          ...
          ...(plainText ? [] : [EditorView.domEventHandlers({
            paste(event, view) {
    ...
    return {
      ...
      exec: (name) => {
        ...
        if (!plainText && command) { command(view); view.focus() }
      },
